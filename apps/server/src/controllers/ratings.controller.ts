...
export const addRating = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(201).json({ message: "note ajouté", note: newRating.rate })
  } catch (error) {
...
export const getAllRatings = async (req: Request, res: Response) => {
  try {
    const allRatings = await ratingService.findAllRatings();
...
export const updateOneRating = async (req: Request<{ id: string }>, res: Response) => {
  try {
    const { id } = req.params;
    const updatedRating = await ratingService.updateById(id, req.body);
